...
class ScheduleMeetingProxyView(APIView):
    ...
    def post(self, request):
        api_key = os.getenv('APPLICATION_1_API_KEY')
        if not api_key:
            return Response({"error": "APPLICATION_1_API_KEY is not configured in backend .env file."}, status=status.HTTP_400_BAD_REQUEST)
